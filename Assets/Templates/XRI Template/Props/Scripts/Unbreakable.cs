...
namespace Unity.XRContent.Interaction
{
    ...
    public class Unbreakable : MonoBehaviour
    {
        ...
        struct ChildPoses
        {
            internal Pose startPose;
            internal Pose endPose;
        }
        ...
        void Start()
        {
            ...
            {
                m_ChildPoses.Add(child, new ChildPoses { startPose = new Pose(child.position, child.rotation) });
            }
        }
            ...
            if (m_Resting)
            {
                if (m_Timer > m_RestTime)
                {
                    ...
                    foreach (var child in m_ChildTransforms)
                    {
                        ...
                        poses.endPose = new Pose(child.position, child.rotation);
            ...
            else
            {
                ...
                else
                {
                    ...
                    foreach (var child in m_ChildTransforms)
                    {
                        ...
                        var poses = m_ChildPoses[child];
                        var lerpedPosition = Vector3.Lerp(poses.endPose.position, poses.startPose.position, timePercent);
                        var lerpedRotation = Quaternion.Slerp(poses.endPose.rotation, poses.startPose.rotation, timePercent);
